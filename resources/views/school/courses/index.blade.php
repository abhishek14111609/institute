        ...
        <div class="row g-4">
                    ...
                    <div
                        ...
                        <div class="card-body p-4 pt-5">
                            <div class="bg-primary bg-opacity-10 p-3 rounded-4 d-inline-block text-primary mb-4">
                                <i
                                    class="bi {{ auth()->user()->school->institute_type === 'sport' ? 'bi-trophy-fill' : 'bi-book-half' }} fs-3"></i>
                            </div>
                            ...
                            <p class="text-muted small mb-4"
                                ...
                                {{ $course->description ?? 'No high-level description provided for this ' . (auth()->user()->school->institute_type === 'sport' ? 'sport' : 'academic program') . '.' }}
                            </p>
                            ...
                            <div class="d-flex gap-2 justify-content-end">
                                ...
                                <form action="{{ route('school.courses.destroy', $course) }}" method="POST"
                                    class="d-inline"
                                    onsubmit="return confirm('{{ auth()->user()->school->institute_type === 'sport' ? 'Deleting this sport will detach all associated classes. Proceed?' : 'Deleting this program will detach all associated classes. Proceed?' }}')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-sm btn-outline-danger rounded-pill px-3"
                                        title="{{ auth()->user()->school->institute_type === 'sport' ? 'Archive Sport' : 'Archive Program' }}">
            ...
            @empty
                <div class="col-12 text-center py-5">
                    ...
                    <h4 class="text-muted">
                        {{ auth()->user()->school->institute_type === 'sport' ? 'No sports added yet.' : 'No academic programs registered.' }}
                    </h4>
                    <p class="text-muted small mb-4">Start organizing your
                        {{ auth()->user()->school->institute_type === 'sport' ? 'training regimen' : 'curriculum' }} by
                        creating
                        {{ auth()->user()->school->institute_type === 'sport' ? 'a new sport.' : 'a new program track.' }}
                    </p>
                    <a href="{{ route('school.courses.create') }}"
                        class="btn btn-primary rounded-pill px-5 shadow-sm border-0">
                        Get Started Now
                    </a>
                </div>
            @endforelse
