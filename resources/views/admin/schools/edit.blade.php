    ...
    <div class="container-fluid">
        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-center mb-5 fade-in">
            <div class="mb-3 mb-md-0">
                ...
                <p class="text-muted mb-0">Synchronizing operational parameters and locality for <span
                        class="fw-bold text-primary">{{ $school->name }}</span>.</p>
            </div>
            <div>
                    ...
                    <div class="card border-0 shadow-sm rounded-4 mb-4 overflow-hidden">
                        ...
                        <div class="card-body p-4">
                            <div class="row g-4">
                                ...
                                <div class="col-md-6">
                                    <label class="form-label small fw-bold text-uppercase text-muted">Operational
                                        Status</label>
                                    <div class="input-group">
                                        <span class="input-group-text bg-light border-0"><i
                                        ...
                                        <select name="status"
                                            ...
                                            required>
                                            <option value="active"
                                                {{ old('status', $school->status) === 'active' ? 'selected' : '' }}>
                                                Institutional Active</option>
                                            <option value="inactive"
                                                {{ old('status', $school->status) === 'inactive' ? 'selected' : '' }}>
                                                Suspended</option>
                                        </select>
                                    </div>
                                    @error('status')
                                    ...
                                    <div class="input-group">
                                        ...
                                        <select name="institute_type"
                                            ...
                                            <option value="academic"
                                                {{ old('institute_type', $school->institute_type) === 'academic' ? 'selected' : '' }}>
                                                Academic</option>
                                            <option value="sport"
                                                {{ old('institute_type', $school->institute_type) === 'sport' ? 'selected' : '' }}>
                                                Sport</option>
                                        </select>
                                    </div>
                        ...
                        <div class="card-body p-4">
                            <div class="d-flex align-items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-muted small mb-3">Institutional subscriptions are managed via specialized
                                        tactical vectors. Modification of price tiers or duration should be executed through
                                        the extension terminal.</p>
                                    <div class="row g-3">
                                        <div class="col-md-6">
                                            <div
                                                ...
                                                <span
                                                    class="badge bg-indigo bg-opacity-10 text-indigo rounded-pill px-3">{{ $school->activeSubscription?->plan->name ?? 'No active subscription' }}</span>
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div
                                                ...
                                                <span
                                                    class="small fw-bold {{ $school->subscription_expires_at?->isPast() ? 'text-danger' : 'text-dark' }}">
                                    ...
                                    <div class="mt-3">
                                        <a href="{{ route('admin.schools.index') }}"
                                            class="btn btn-sm btn-info text-white fw-bold rounded-pill px-4">Initialize
                ...
                <div class="col-lg-4">
                    <div class="card border-0 shadow-sm rounded-4 mb-4 overflow-hidden">
                        ...
                        <div class="card-body p-4 text-center">
                            <div class="mb-4">
                                <div class="bg-primary bg-opacity-10 rounded-circle d-inline-flex align-items-center justify-content-center mb-3 shadow-inner position-relative overflow-hidden"
                                    style="width: 150px; height: 150px; border: 4px dashed rgba(79, 70, 229, 0.2);">
                                    @if ($school->logo)
                                        <img src="{{ route('media.public', ['path' => $school->logo]) }}"
                            ...
                            @error('logo')
                                <div class="invalid-feedback d-block mt-1">{{ $message }}</div>
                            @enderror
                            <small class="text-muted tiny mt-2 d-block">Leave empty to maintain existing brand
                                identity.</small>
                        </div>
                    </div>
                    ...
                    <div class="d-flex flex-column gap-3">
                        ...
                        <a href="{{ route('admin.schools.index') }}"
                            class="btn btn-light btn-lg rounded-pill fw-bold px-4 py-3 border w-100">
                            Cancel
                        </a>
                    </div>

                    <div
                        class="card border-0 shadow-sm rounded-4 mt-4 bg-danger bg-opacity-10 border border-danger border-opacity-10 overflow-hidden">
                        <div class="card-body p-4 text-center">
                            <h6 class="fw-bold text-danger mb-2">Institutional Termination</h6>
                            <p class="text-danger tiny mb-4 opacity-75">Warning: Decommissioning this node will permanently
                                eradicate all associated data networks, users, and historical records.</p>
                            <form action="{{ route('admin.schools.destroy', $school) }}" method="POST"
                                onsubmit="return confirm('CRITICAL ACTION: Are you sure you want to decommission this institutional node? This operation is irreversible.');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-danger w-100 rounded-pill py-2 fw-bold">Execute
