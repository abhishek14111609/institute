...
@section('content')
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2>{{ auth()->user()->school->institute_type === 'sport' ? 'Edit Coach/Staff:' : 'Edit Teacher:' }}
                {{ $teacher->user->name }}</h2>
        ...
        <div class="card">
            <div class="card-body">
                <form action="{{ route('school.teachers.update', $teacher) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="col-md-4">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('username') is-invalid @enderror"
                                    ...
                                    data-rules="required|alpha_dash|min:3" data-user-id="{{ $teacher->user_id }}">
                                @error('username')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <h5 class="mb-3">
                        {{ auth()->user()->school->institute_type === 'sport' ? 'Coaching/Staff Information' : 'Professional Information' }}
                    </h5>
                    <div class="row">
                        <div class="col-md-4">
                            <div class="mb-3">
                                <label for="employee_id"
                                    class="form-label">{{ auth()->user()->school->institute_type === 'sport' ? 'Staff/Coach ID' : 'Employee ID' }}</label>
                                <input type="text" class="form-control bg-light" id="employee_id" name="employee_id"
                                    value="{{ old('employee_id', $teacher->employee_id) }}" readonly>
                                @error('employee_id')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-4">
                            <div class="mb-3">
                                ...
                                <input type="date" class="form-control @error('joining_date') is-invalid @enderror"
                                    ...
                                    value="{{ old('joining_date', $teacher->joining_date->format('Y-m-d')) }}" required>
                                @error('joining_date')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="mb-2">
                            @if ($teacher->user->avatar)
                                <img src="{{ route('media.public', ['path' => $teacher->user->avatar]) }}" alt="Avatar"
                    ...
                    <div class="mb-3">
                        <label
                            class="form-label">{{ auth()->user()->school->institute_type === 'sport' ? 'Assign Sessions' : 'Assign Batches' }}</label>
                        <div class="border rounded p-3 bg-light" style="max-height: 200px; overflow-y: auto;">
                            @foreach ($batches as $batch)
                                <div class="form-check mb-2">
                                    <input class="form-check-input @error('batches') is-invalid @enderror"
                                        type="checkbox" name="batches[]" value="{{ $batch->id }}"
                                        id="batch_{{ $batch->id }}"
                                        {{ in_array($batch->id, old('batches', $teacher->batches->pluck('id')->toArray())) ? 'checked' : '' }}>
                                    <label class="form-check-label" for="batch_{{ $batch->id }}">
                                        {{ $batch->name }} <span
                                            class="text-muted small">({{ $batch->class->name }})</span>
                                    </label>
                                </div>
                            @endforeach
                        </div>
                        <small class="text-muted">Select one or more
                            {{ auth()->user()->school->institute_type === 'sport' ? 'sessions' : 'batches' }} by checking
                            the boxes</small>
                        @error('batches')
                    ...
                    <div class="d-flex gap-2">
                        <button type="submit" class="btn btn-primary">
                            ...
                            {{ auth()->user()->school->institute_type === 'sport' ? 'Update Coach' : 'Update Teacher' }}
                        </button>
