...
@section('content')
            ...
            <div class="col-xl-6 mx-auto">
                <div class="card border-0 shadow-lg rounded-4 overflow-hidden">
                    <div class="card-header bg-primary text-white border-0 py-4 px-4">
                        <h4 class="fw-bold mb-0"><i class="bi bi-pencil-square me-2"></i> Update Stock: {{ $inventory->name }}
                        </h4>
                    ...
                    <form action="{{ route('school.inventory.update', $inventory) }}" method="POST">
                                    ...
                                    <select name="category" class="form-select" required>
                                        ...
                                        @foreach ($categories ?? collect() as $categoryItem)
                                            <option value="{{ $categoryItem->name }}"
                                                {{ $inventory->category == $categoryItem->name ? 'selected' : '' }}>
                                                {{ $categoryItem->name }}
                                            </option>
                                        @endforeach
                                        @if (($categories ?? collect())->where('name', $inventory->category)->isEmpty())
                                            <option value="{{ $inventory->category }}" selected>{{ $inventory->category }}
                                            </option>
                                        @endif
                                        <option value="__new__">+ Add New Category</option>
                                    </select>
                                    <input type="text" name="new_category" class="form-control mt-2 d-none"
                                        placeholder="Enter new stock category" id="inventory_edit_new_category">
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label class="form-label small fw-bold text-muted">UNIT PRICE (₹)</label>
                                    <input type="number" name="price" class="form-control"
                                        value="{{ $inventory->price }}" step="0.01" required>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="number" name="stock_quantity" class="form-control"
                                        value="{{ $inventory->stock_quantity }}" required>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="number" name="alert_quantity" class="form-control"
                                        value="{{ $inventory->alert_quantity }}">
                                </div>
                            </div>

                            <div class="mb-0">
                                <label class="form-label small fw-bold text-muted">ATTACH TO COURSE / CLASS
                                    (OPTIONAL)</label>
                                <select name="course_id" class="form-select">
                                    <option value="">— Not specific —</option>
                                    @foreach ($courses as $course)
                                        <option value="{{ $course->id }}"
                                            {{ $inventory->course_id == $course->id ? 'selected' : '' }}>
                                            {{ $course->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="card-footer border-0 p-4 pt-0 bg-white">
                            ...
                            <div class="d-flex justify-content-end gap-2">
                                ...
                                <button type="submit" class="btn btn-primary rounded-pill px-5 shadow">Update
                                    Records</button>
                            </div>
                        </div>
                    </form>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const newCategoryInput = document.getElementById('inventory_edit_new_category');
