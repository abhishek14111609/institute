...
@section('content')
        ...
        <div class="row g-4 m-0">
            <div class="col-12">
                <div class="d-flex justify-content-between align-items-center mb-2">
                    <div>
                        <h3 class="fw-bold mb-1 text-primary">{{ $isSport ? 'Kits & Equipment Stock' : 'Inventory & Stock Room' }}</h3>
                        <p class="text-muted small">Manage physical inventory, sell items to students, and generate instant invoices.</p>
                    </div>
                    <div>
                        <button class="btn btn-primary rounded-pill px-4 shadow-sm fw-bold me-2" data-bs-toggle="modal" data-bs-target="#addItemModal">
                            <i class="bi bi-plus-lg me-2"></i> Add New Item
                        </button>
                        <button class="btn btn-success rounded-pill px-4 shadow-sm fw-bold" data-bs-toggle="modal" data-bs-target="#issueItemModal">
                ...
                <div class="card border-0 shadow-sm rounded-4 bg-danger text-white">
                    <div class="card-body p-4 d-flex align-items-center">
                        ...
                        <div>
                            ...
                            <h2 class="fw-bold mb-0">{{ $items->filter(fn($i) => $i->stock_quantity <= ($i->alert_quantity ?? 5))->count() }}</h2>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card border-0 shadow-sm rounded-4 bg-info text-dark">
                    <div class="card-body p-4 d-flex align-items-center">
                        ...
                        <div>
                            ...
                            <h2 class="fw-bold mb-0">&#8377;{{ number_format($items->sum(function ($i) { return $i->price * $i->stock_quantity; }), 0) }}</h2>
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            <div class="row g-3 align-items-center">
                                ...
                                <div class="col-md-4">
                                    <div class="input-group input-group-sm">
                                        ...
                                        <input type="text" name="search" class="form-control border-0 bg-light" placeholder="Search by name..." value="{{ request('search') }}">
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    <select class="form-select form-select-sm border-0 bg-light" name="category" onchange="this.form.submit()">
                                        <option value="All" {{ request('category') == 'All' ? 'selected' : '' }}>All Categories</option>
                                        <option value="Kit" {{ request('category') == 'Kit' ? 'selected' : '' }}>Kits & Gear</option>
                                        <option value="Book" {{ request('category') == 'Book' ? 'selected' : '' }}>Books</option>
                                        <option value="Uniform" {{ request('category') == 'Uniform' ? 'selected' : '' }}>Uniforms</option>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center border-light">
                                                @if($item->stock_quantity <= ($item->alert_quantity ?? 5))
                                                    <span class="badge bg-danger rounded-pill px-3">{{ $item->stock_quantity }} Low Stock</span>
                                                @else
                                                    <span class="fw-bold">{{ $item->stock_quantity }}</span>
                                                @endif
                                            </td>
                                            <td class="border-light text-center">&#8377;{{ number_format($item->price, 2) }}</td>
                                            <td class="border-light text-center fw-bold text-primary">&#8377;{{ number_format($item->price * $item->stock_quantity) }}</td>
                                            <td class="text-end pe-0 border-light">
                                                <div class="dropdown">
                                                    ...
                                                    <ul class="dropdown-menu dropdown-menu-end border-0 shadow-lg">
                                                        <li><a class="dropdown-item py-2" href="{{ route('school.inventory.edit', $item) }}"><i class="bi bi-pencil me-2 text-primary"></i> Edit Stock</a></li>
                                                        <li><hr class="dropdown-divider"></li>
                                                        <li>
                                                            <form action="{{ route('school.inventory.destroy', $item) }}" method="POST" onsubmit="return confirm('Remove this item from store?')">
                                                                @csrf
                                                                @method('DELETE')
                                ...
                                <div class="list-group-item bg-light border-0 rounded-4 p-3 mb-2">
                                    <div class="d-flex justify-content-between align-items-start mb-2">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <h6 class="fw-bold mb-0 small text-dark truncate-1" title="{{ $sale->student->user->name ?? 'Unknown' }}">{{ $sale->student->user->name ?? 'Unknown' }}</h6>
                                        </div>
                                        <span class="badge {{ ($sale->payment_status ?? 'unpaid') == 'paid' ? 'bg-success' : 'bg-warning' }} rounded-pill x-small">
                                            {{ strtoupper($sale->payment_status ?? 'UNPAID') }}
                                        </span>
                                    </div>
                                    <p class="small text-muted mb-1 truncate-1">{{ $sale->quantity }}x {{ $sale->item->name ?? 'Deleted Item' }}</p>
                                    <div class="d-flex justify-content-between align-items-center border-top pt-2 mt-2">
                                        <div>
                                            <span class="fw-bold text-primary small">&#8377;{{ number_format($sale->total_amount, 2) }}</span>
                                            @if($sale->invoice)
                                                <div class="text-muted x-small">Invoice: {{ $sale->invoice->invoice_number }}</div>
                                            @endif
                                        </div>
                                        <div class="text-end">
                                            <span class="text-muted x-small opacity-75 d-block">{{ $sale->created_at->diffForHumans(null, true) }}</span>
                                            @if($sale->invoice)
                                                <a href="{{ route('school.invoices.stream', $sale->invoice) }}" target="_blank" class="text-decoration-none x-small fw-bold">Open Invoice</a>
                                            @endif
                                        </div>
                                    </div>
                                </div>
    ...
    <style>
        ...
        .list-group-item:hover { transform: translateY(-3px); box-shadow: 0 .5rem 1rem rgba(0,0,0,.05); transition: 0.3s cubic-bezier(.4, 0, .2, 1); cursor: pointer; }
