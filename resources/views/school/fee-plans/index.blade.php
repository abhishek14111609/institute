...
@section('content')
    <div class="container-fluid py-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-5 pb-2">
            ...
            <a href="{{ route('school.fee-plans.create') }}"
        ...
        @else
            <div class="row g-4 mb-4">
                @foreach ($plans as $plan)
                    @php
                        $cat = $categoryMap[$plan->fee_type] ?? [
                            'label' => ucfirst($plan->fee_type),
                            'icon' => 'bi-tag-fill',
                            'color' => 'bg-secondary',
                        ];
                        $dur = $plan->duration
                            ? $durationMap[$plan->duration] ?? [
                                    'label' => ucfirst($plan->duration),
                    ...
                    <div class="col-md-4">
                        <div
                            class="card border-0 shadow-sm rounded-4 h-100 hover-lift transition-all overflow-hidden {{ !$plan->is_active ? 'opacity-75 grayscale' : '' }}">
                            <div class="card-body p-4">
                                <div class="d-flex justify-content-between align-items-start mb-4">
                                    ...
                                    <span
                                        class="badge {{ $plan->is_active ? 'bg-success' : 'bg-secondary' }} rounded-pill px-3 py-1 tiny fw-bold shadow-sm">
                                ...
                                <div class="d-flex flex-wrap gap-2 mb-4">
                                    <span
                                        class="badge {{ $cat['color'] }} rounded-pill px-3 py-1 tiny fw-bold">{{ $cat['label'] }}</span>
                                    @if ($dur)
                                        <span
                                            class="badge {{ $dur['color'] }} rounded-pill px-3 py-1 tiny fw-bold">{{ $dur['label'] }}</span>
                                    @endif
                                    @if ($plan->sport_level)
                                        <span
                                            class="badge bg-purple-soft text-purple rounded-pill px-3 py-1 tiny fw-bold shadow-none">
                                            <i class="bi bi-award me-1"></i> {{ ucfirst($plan->sport_level) }} Tier
                                        </span>
                                    @endif
                                    @if ($plan->course)
                                        <span
                                            class="badge bg-info-subtle text-info rounded-pill px-3 py-1 tiny fw-bold shadow-none">
                                            <i class="bi bi-book-half me-1"></i> {{ $plan->course->name }}
                                        </span>
                                    @endif
                                    @if ($plan->batch)
                                        <span
                                            class="badge bg-warning-subtle text-dark rounded-pill px-3 py-1 tiny fw-bold shadow-none">
                                ...
                                <div class="mb-4">
                                    <div class="d-flex align-items-baseline gap-1">
                                        <span
                                            class="h2 fw-bold text-primary mb-0">₹{{ number_format($plan->amount, 0) }}</span>
                                    ...
                                    @if ($plan->late_fee_per_day > 0)
                                        <div class="mt-1 d-flex align-items-center text-danger tiny fw-bold">
                                            <i class="bi bi-clock-history me-1"></i> Late Penalty:
                                            ₹{{ number_format($plan->late_fee_per_day, 0) }} / day
                                        </div>
                                    @endif
                                </div>

                                @if ($plan->description)
                                    <p class="text-muted tiny mb-4 line-clamp-2" style="min-height: 2.5rem;">
                                        {{ $plan->description }}</p>
                                @else
                                    <div class="mb-4 text-muted tiny font-italic opacity-50" style="min-height: 2.5rem;">No
                                        institutional guidelines provided for this plan.</div>
                                @endif

                                <div class="d-flex gap-2 pt-3 border-top mt-auto">
                                    <a href="{{ route('school.fees.create', ['plan' => $plan->id]) }}"
                                        class="btn btn-primary grow rounded-pill py-2 small fw-bold shadow-sm border-0">
                                        <i class="bi bi-node-plus-fill me-1"></i> Issue to Student
                                    </a>
                                    <div class="btn-group shadow-sm rounded-pill overflow-hidden">
                                        <a href="{{ route('school.fee-plans.edit', $plan) }}"
                                            class="btn btn-light border-0 px-3" title="Edit Template">
                                            <i class="bi bi-pencil-square text-warning"></i>
                                        </a>
                                        @if ($plan->fees_count > 0)
                                            <button type="button" class="btn btn-light border-0 px-3 opacity-25" disabled
                                                title="Plan in Use: {{ $plan->fees_count }} Ledger Links">
                                                <i class="bi bi-trash3 opacity-50"></i>
                                            </button>
                                        @else
                                            <form action="{{ route('school.fee-plans.destroy', $plan) }}" method="POST"
                                                onsubmit="return confirm('Archive this billing artifact?')"
                                                class="d-inline">
                                                @csrf @method('DELETE')
